												...
												<table
													...
													<tbody>
														@foreach($data as $key=> $row)
                                                            @php $now = Carbon\Carbon::now()->timezone('Asia/Tehran')->format("Y-m-d H:i"); @endphp
                                                            <tr role="row" class="odd" @if(isset($row['to_date']) && @$row['to_date'] < $now ) style="background-color: #c7ddef" @endif>
															<td class="sorting_1">
																<input
																	style="opacity: 1;position:static;"
																	name="deleteId[]"
                                                            ...
                                                            <td class="sorting_1">
                                                                {{$row->used != 0 ? 'استفاده شده' : 'استفاده نشده '}}
                                                                @if($row->kind == 2)

                                                                    (تعداد باقی مانده: {{intval($row->count - $row->used )}})
                                                                @endif
                                                            </td>
                                                            <td class="sorting_1">
