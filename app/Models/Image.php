...
class Image extends Model
{
    use SoftDeletes;
   protected $fillable = [
     'old_file','file' , 'product_id','thumbnail','product_specification_id','old_id','convert','fix_pic','sort','webp','file2','error','img','main_file'
     ,'jpg','file3','file_hamide','replaced'
   ];
    ...
    public function getProImageAttribute(){
      ...
      public function getSmallImageAttribute(){
        if(file_exists('assets/uploads/content/pro/small/'.@$this->attributes['file'])){
            return asset('assets/uploads/content/pro/small/'.@$this->attributes['file']);

        }elseif(file_exists('assets/uploads/content/sp/small/'.@$this->attributes['file'])){
            return asset('assets/uploads/content/sp/small/'.@$this->attributes['file']);
