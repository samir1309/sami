...
<div class="container-fluid  dashboard-content">
	...
	<div class="row w-100 m-0">
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 px-0 d-flex align-items-center justify-content-between bg-white">
			...
			<a href="{{url('admin/products/')}}" class="btn btn-default btn-info m-0 rounded-0" style="float: left" data-toggle="tooltip" data-placement="top" title="" data-original-title="بازگشت">
				بازگشت
	...
	<br>
	<div class="card">
        @include('admin.product_image.add')
		<form method="post" action="{{URL::action('Admin\ProductController@postDeleteImage')}}">
			{{ csrf_field() }}
			<div class="px-2 py-3">
...
{{--				<a href="{{URL::action('Admin\ProductController@getAddImage' , $image->id)}}" type="button"--}}
									...
									<div class="row w-100 m-0">
										<div class="col-sm-8">
											<table class="table table-striped table-bordered first dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
												<thead>
													<tr role="row">
														<th class="sorting_asc text-start" tabindex="0"
															aria-controls="DataTables_Table_0"
															rowspan="1" colspan="1"
															aria-sort="ascending"
												...
												<tbody>
													...
													<tr role="row" class="odd">
														<td class="sorting_1 text-start">
															<input style="opacity: 1;position:static;" name="deleteId[]" class="delete-all" type="checkbox" value="{{$row->id}}" />
														</td>
														<td class="sorting_1 text-center">
															{{$key+1}}
														</td>
														<td class="sorting_1 text-center">
															<img class="mx-auto swiper-lazy" width="60" @if(file_exists('assets/uploads/content/pro/big/'.@$row->file)) src="{{asset('assets/uploads/content/pro/big/'.$row->file)}}" @else src="{{asset('assets/site/images/notfound.png')}}" @endif>
														</td>

															<td class="sorting_1 text-center">
                                                            @if($row->thumbnail == 0)
                                                                <a class="btn btn-success m-0 rounded-0 me-2" href="{{URL::action('Admin\ProductController@editThumbnail',$row->id)}}" onclick="this.disabled=true;this.value='Submitting...'; this.form.submit();">انتخاب به عنوان مشخصه</a>
                                                            @else
                                                                <a class="btn btn-secondary m-0 rounded-0 me-2" href="{{URL::action('Admin\ProductController@editThumbnail',$row->id)}}" onclick="this.disabled=true;this.value='Submitting...'; this.form.submit();">عدم انتخاب به عنوان مشخصه</a>
                                                            @endif
                                                                @if($row->thumbnail == 1)
                                                                <span class='label label-success'> تصویر کاور</span>
                                                                @else
                                                                    <span class='label label-danger'> تصویر معمولی</span>
                                                                @endif
														</td>
													</tr>
													@endforeach
                                                ...
                                                <ul>
                                                    ...
                                                    @foreach($categorySort as $rowSort)
                                                        <li class="list-unstyled  my-2 p-3 shadow-sm rounded-lg" id="arrayorder_{!! stripslashes($rowSort['id']) !!}"  style="background-color: #333" >
                                                            <img src="{{asset('assets/uploads/content/pro/big/'.$rowSort['file'])}}" class="w-100" style="height: 150px;">
                                                            <div class="clear"></div>
                                                        </li>
...
@section('js')
    ...
    <script>
        new Vue ({
            ...
            methods: {
                uploadSuccess(id, response, file, imgList){
                    //upload images of a gallery
                    var vm = this;
                    vm.imgList = !imgList;
                    // setTimeout(()=>{
