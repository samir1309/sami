...
class Helper
{
      public static function changePriceTable($product_id){
        ...
        $product->update([
            'price'=>intval(@$product->price_second['price']),
            'old_price'=>intval(@$product->price_second['old']),
        ]);
        return true;
    }
    public static function changePrice($product_id){
        $product = Product::find($product_id);
        $product->update([
            'price'=>intval(@$product->price_third['price']),
            'old_price'=>intval(@$product->price_third['old']),
        ]);
        return true;
    }
