...
<div class="container-fluid  dashboard-content">
	...
	<div class="card">
		<form method="post" action="{{url('/admin/price-discount/delete')}}">
			{{ csrf_field() }}
			<div class="py-3 px-2">
				...
				<a href="{{url('admin/price-discount')}}" type="button" class="btn btn-space btn-primary">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
											...
											<table class="table table-striped table-bordered first dataTable"
												...
												<tbody>
													...
													<tr role="row" class="odd">
														...
														<td class="sorting_1">{{@$row->percent}}</td>
														<td class="sorting_1">
                                                            @if(@$row->brand_id != null)
                                                          {{ 'برند:'.@$row->brand->title}}
                                                            @elseif(@$row->category_id != null)
                                                                {{ 'دسته:'.@$row->category->title}}
                                                            @endif
                                                        </td>
														<td>

                                                            <a href="{{URL::action('Admin\PriceController@getDeactive',@$row->id)}}"
                                                               type="button" onclick="return confirm('آیا از غیرفعال کردن تخفیف مطمئن هستید؟');"
